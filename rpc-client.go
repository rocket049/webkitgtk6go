...
func (p *MyRpcClient) Call(conn *websocket.Conn, fn string, args interface{}) <-chan interface{} {
	p.Conn = conn
	id := p.Id.Add(1)
	cmd := &RpcCmd{
		Typ:    "call",
...
func (p *MyRpcClient) Notify(conn *websocket.Conn, fn string, args interface{}) {
	p.Conn = conn
